(function ($) {
    ...
    handbid.alert = function (message, cb) {
        handbid._confirmCallback = cb;

        var confirmationMarkup = '<div id="handbid-confirmation-underlay" ' +
            'style="position: fixed; top:0px; bottom:0px; left: 0px; right: 0px;">' +
            '<div id="handbid-confirmation-dialog" style="position: fixed; left:50%; top:50%;">' +
            '<div class="handbid-message">' + message + '</div>' +
            '<div class="handbid-buttons"><a class="handbid-ok" href="#" onclick="handbid.confirmYes(); return false;">Ok</a>' +
            '</div>' +
            '</div>';

        $('body').append(confirmationMarkup);

        //center dialog
        var dlg = $('#handbid-confirmation-dialog');


        dlg.css({
            marginLeft: -1 * dlg.outerWidth(true) / 2 + 'px',
            marginTop: -1 * dlg.outerHeight(true) / 2 + 'px'
        });
    ...
    handbid._confirmCallback = null;
    handbid.confirm = function (message, cb) {
        ...
        var confirmationMarkup = '<div id="handbid-confirmation-underlay" ' +
            ...
            '<div id="handbid-confirmation-dialog" style="position: fixed; left:50%; top:50%;">' +
            '<div class="handbid-message">' + message + '</div>' +
            '<div class="handbid-buttons"><a class="handbid-cancel" href="#" onclick="handbid.confirmNo(); return false;">Cancel</a> <a class="handbid-ok" href="#" onclick="handbid.confirmYes(); return false;">Ok</a> ' +
            '</div>' +
            '</div>';
    ...
    handbid.confirmYes = function () {

        if(handbid._confirmCallback) {
            handbid._confirmCallback(true);
        }

        $('#handbid-confirmation-underlay').remove();

    };

    handbid.confirmNo = function () {

        if(handbid._confirmCallback) {
            handbid._confirmCallback(false);
        }
    ...
    handbid.notice = function (msg, cb) {
        ...
        $('.message', growl).html(escapeHtml(msg));
