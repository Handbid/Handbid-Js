...
Sender.prototype.frameAndSend = function(opcode, data, finalFragment, maskData, cb) {
  ...
  if (!data) {
    try {
      this._serverSocket.write(new Buffer([opcode | (finalFragment ? 0x80 : 0), 0 | (maskData ? 0x80 : 0)].concat(maskData ? [0, 0, 0, 0] : [])), 'binary', cb);
    }
    catch (e) {
  ...
  if (maskData) {
    ...
    if (mergeBuffers) {
      ...
      try {
        this._serverSocket.write(outputBuffer, 'binary', cb);
      }
      catch (e) {
        if (typeof cb == 'function') cb(e);
        else this.emit('error', e);
      ...
      try {
        this._serverSocket.write(outputBuffer, 'binary');
        this._serverSocket.write(data, 'binary', cb);
      }
      catch (e) {
        if (typeof cb == 'function') cb(e);
        else this.emit('error', e);
