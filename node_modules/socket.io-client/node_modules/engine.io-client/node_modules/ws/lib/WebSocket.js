...
WebSocket.prototype.pause = function() {
  ...
  return this._serverSocket.pause();
}
...
WebSocket.prototype.resume = function() {
  if (this.readyState != WebSocket.OPEN) throw new Error('not opened');
  return this._serverSocket.resume();
}
...
Object.defineProperty(WebSocket.prototype, 'bufferedAmount', {
  get: function get() {
    var amount = 0;
    if (this._serverSocket) {
      amount = this._serverSocket.bufferSize || 0;
    }
    return amount;
  }
});
...
function cleanupWebsocketResources(error) {
  ...
  if (this._serverSocket) {
    removeAllListeners(this._serverSocket);
    // catch all socket error after removing all standard handlers
    var socket = this._serverSocket;
    this._serverSocket.on('error', function() {
      try { socket.destroy(); } catch (e) {}
    });
    try {
      if (!error) this._serverSocket.end();
      else this._serverSocket.destroy();
    }
